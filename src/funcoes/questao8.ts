...
function controle(preco:number,quantidade:number){
            let valor = preco*quantidade

        if(quantidade > 10){
        let desconto:number = (5/100)*valor
...
let preco:number = Number(prompt('Qual foi o valor  unitário do item?'))
let contt = 0; let acumpreco = 0

while(preco != 0){
    
    let quantidade:number = Number(prompt('Qual foi a quantidade de produtos?'))
    let controle1 =  controle(preco,quantidade)
    acumpreco += controle1
    contt++
    preco = Number(prompt('Qual foi o valor  unitário do item?'))
}
let media: number = acumpreco/contt
